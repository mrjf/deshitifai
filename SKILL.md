---
name: deshitifai
description: Help a user transform an annoying, bloated, hostile, or hard-to-use website into a fast, simple, useful static site. Use when the user wants to deshitify a website, scrape public data into a schema, build a cleaner replacement view, buy or configure a domain, host the result on Cloudflare Pages or GitHub Pages, and stay with the process from idea through tested and completed launch.
---

# Deshitifai

Use this skill to walk a user all the way from "this website is annoying" to a tested, hosted, working static site that presents the useful information cleanly.

The session can be long. Stay with the user through discovery, scraping, schema design, site creation, testing, hosting, domain setup, and final verification. Do not stop at advice if you can keep making concrete progress.
...
Read `principles.md` in this repository before designing the replacement site. Use it as a checklist while making decisions:
...
### 1. Understand the annoyance

Identify the target website, page, or workflow. Ask the user what feels bad about it:
...
Define the useful thing the replacement site should preserve. Write a short working goal in plain language.

### 2. Check feasibility and boundaries

Inspect the source site and determine what can be rebuilt safely and practically.
...
State the boundary: what will be copied, summarized, linked, transformed, omitted, or newly created.
...
Create the simplest useful static site that serves the user's goal.
...
Do not make a marketing landing page when the user asked for a tool, directory, database, index, viewer, or replacement interface. Put the useful experience first.
...
- Does it remove the annoying friction?
...
By the end of a successful run, deliver a working static site that:

- solves the user's original annoyance
